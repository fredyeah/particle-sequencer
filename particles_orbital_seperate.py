...
def animate(t):
    ...
    for p1, p2 in combinations(particles[1:], 2):
        x = p1.x - p2.x
        y = p1.y - p2.y
        z = math.sqrt(x**2 + y**2)
        if z < (p1.size + p2.size):
            pass
            # p1.patch.remove()
            # p2.size = (p2.size + p1.size) * 0.8
            # p2.patch.set_radius(p2.size)
            # ax.patches.remove(p1.patch)
            # particles.remove(p1)
            # particles.pop()
            # particles[0].patch.set_color((random.random(), random.random(), random.random()))
    #         x = p1.x - p2.x
    #         y = p1.y - p2.y
